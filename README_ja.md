...
環境変数を使用して、GTM Copilotの動作をカスタマイズできます。これらの変数は、**プロジェクトのルートディレクトリ**（またはスキルディレクトリ）にある `.env` ファイルに定義してください：

- `GTM_CLIENT_ID`: Google Cloud Consoleで取得したOAuth 2.0 クライアントID。
- `GTM_CLIENT_SECRET`: Google Cloud Consoleで取得したOAuth 2.0 クライアントシークレット。
- `GTM_REFRESH_TOKEN`: `auth.py` を実行して生成されたOAuth 2.0 リフレッシュトークン。アクセストークンの自動更新に使用されます。
- `GTM_EXPORT_ROOT_PATH`: GTM設定の書き出しおよび読み込み時のルートディレクトリを定義します。
  - **プレースホルダー**: `[[GTM_ID]]` をサポートしており、実行時にコンテナの公開ID（例: `GTM-XXXXXX`）に自動置換されます。
  - **デフォルト**: `tmp/[[GTM_ID]]`
  - **優先順位**: コマンドライン引数 > 環境変数 > デフォルトパス
